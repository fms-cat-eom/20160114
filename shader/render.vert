...
void main() {
  vec3 pos = texture2D( particleTexture, ( uv.xy * vec2( 4.0, 1.0 ) + vec2( 0.5, 0.5 ) ) / vec2( 4.0, 1.0 ) / particleCountSqrt ).xyz;
  vec3 vel = texture2D( particleTexture, ( uv.xy * vec2( 4.0, 1.0 ) + vec2( 1.5, 0.5 ) ) / vec2( 4.0, 1.0 ) / particleCountSqrt ).xyz;
  vec3 life = texture2D( particleTexture, ( uv.xy * vec2( 4.0, 1.0 ) + vec2( 2.5, 0.5 ) ) / vec2( 4.0, 1.0 ) / particleCountSqrt ).xyz;
  vec3 nouse = texture2D( particleTexture, ( uv.xy * vec2( 4.0, 1.0 ) + vec2( 3.5, 0.5 ) ) / vec2( 4.0, 1.0 ) / particleCountSqrt ).xyz;

  vBrig = sin( life.x / 0.99 * PI * 1.0 );
  if ( 0.99 < life.x ) {
    vBrig = 0.0;
  }

  mat4 matP = perspective( 60.0, resolution.x / resolution.y, 0.01, 100.0 );
  mat4 matV = lookAt( vec3( 0.0, 0.0, 1.0 ), vec3( 0.0, 0.0, 0.0 ), vec3( 0.0, 1.0, 0.0 ) );
